...
public class RDFPredefinedMappingsLoaderServiceImpl implements RDFPredefinedMappingsLoaderService {
	...
	private void loadMappingsBetweenDBTuneIntegrAndMagnatuneMusicArtistWR() {
		...
		//targetMusicArtist --> This is the view that I want.. 

		//load1t1DNSCMappingBetweenTwoSchemas("dbTuneIntegrSchema", "magnatuneRDFSchema", "MusicArtist", "MusicArtist", targetMusicArtist, 1); //this isn't a DNSC, unless you still have problems with the upper and lower case ... it should be a SNSC, which would make things easier
	...
	private void load1t1SNSCMappingBetweenTwoSchemas(String sourceSchemaName, String targetSchemaName, String sourceSaName, String targetSaName,
		...
		//sourceSchemaName --> is my global schema, right? - no, the source from where the data comes is the sourceSchema, the targetSchema is the one that's populated with the data from the source
		//targetSchemaName
		...
		//String targetQueryString = "SELECT * FROM " + targetSaName; //<--- Here do I select only the attributes that I want. - yes, add the attributes you want in the SELECT clause - see my class for example, don't use as if possible
